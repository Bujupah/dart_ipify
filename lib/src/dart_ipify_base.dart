...
class Ipify {
  ...
  /// ``` 
  static Future<String> ipv4({Format format = Format.TEXT}) async => await _send(_hostv4, format);
  ...
  /// ``` 
  static Future<String> ipv64({Format format = Format.TEXT}) async => await _send(_hostv64, format);
  
  static Future<String> _send(String host, Format format) async {
    
    try {
      
      final uri = Uri(host: host, path: _path, scheme: _scheme, query: _param(format));
  ...
  static String _param(Format format) => _format + (Format.TEXT == format ? '' : Format.JSON == format ? 'json' : 'jsonp');  
}